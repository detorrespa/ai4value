  ...
  return (
    <nav className="border-b border-white/5">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <a href="mailto:alberto@nektiu.com" className="text-xs text-slate-400 hover:text-teal-500 transition-colors">
          alberto@nektiu.com
        </a>
      </div>
    </nav>
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-20 animate-fade-in">
      <div className="inline-flex items-center gap-2 bg-teal-500/10 text-teal-500 px-3 py-1 rounded-full mb-8">
        <Sparkles size={12} />
        <span className="text-[11px] font-semibold tracking-wider uppercase">Diagnóstico · 7 minutos</span>
      </div>
      ...
      <p className="text-slate-400 text-lg leading-relaxed max-w-2xl">
        ...
        comparado con la media de tu sector y recibirás un <strong className="text-white">informe Word personalizado</strong>
        {" "}con tu plan de acción a 90 días, presupuesto estimado y KPIs.
      </p>
    ...
    <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in">
      <div className="mb-10">
        ...
        <div className="h-[3px] bg-white/5 rounded-full overflow-hidden">
          <div className="h-full bg-teal-500 transition-all duration-400" style={{ width: `${progress}%` }} />
        </div>
      </div>
                ...
                {[1, 2, 3, 4, 5].map(v => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[9px] leading-tight ${isSelected ? "text-navy-900/70" : "text-slate-500"}`}>
                        {LIKERT_LABELS[v - 1]}
                      </span>
                    </button>
                  );
...
function PreviewScreen({ scores, onUnlock }: { scores: Scores; onUnlock: () => void }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in">
      <div className="text-center mb-10">
        ...
        <h1 className="font-serif text-4xl md:text-5xl mb-4">Tu puntuación de madurez IA</h1>
      </div>
      ...
      <div className="card p-10 text-center mb-8">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border" style={{ borderColor: `${level.color}40` }}>
        ...
        <p className="text-slate-400 mt-4 max-w-md mx-auto text-sm leading-relaxed">{level.desc}</p>
      </div>

      <div className="relative">
        <div className="card p-8 space-y-4">
          <h2 className="font-serif text-2xl text-center">Desbloquea tu informe personalizado</h2>
          <p className="text-slate-400 text-center max-w-xl mx-auto">
            Te enviamos por email tu <strong className="text-white">informe Word personalizado</strong> de 8 páginas
            con el análisis detallado, tu comparativa con la media del sector, plan de acción a 90 días y presupuesto estimado.
          </p>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 py-6">
            <PreviewFeature icon={Target} label="Pentágono vs sector" />
            <PreviewFeature icon={FileDown} label="Informe Word · 8 páginas" />
            <PreviewFeature icon={Clock} label="Plan 90 días" />
            <PreviewFeature icon={Rocket} label="Presupuesto + KPIs" />
          </div>
      ...
      <div className="fixed inset-0 bg-navy-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
        <div className="card p-10 max-w-md text-center">
          ...
          <p className="text-slate-400 mb-3">
            Hemos generado tu informe personalizado y te lo hemos enviado a
            <span className="text-white font-semibold"> {profile.email}</span>.
          </p>
          <p className="text-sm text-slate-500">Cargando tus resultados…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="card p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400">
            Dinos a quién se lo enviamos. Lo generamos en el momento con tus datos.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Rol">
              <input className="input" placeholder="Director General"
          ...
          <Field label="Sector" required error={errors.sector}>
            <select className="input" value={profile.sector} onChange={e => setProfile({ ...profile, sector: e.target.value })}>
                ...
                return (
                  <button key={s.id} type="button"
                    ...
                    className={`text-left px-3 py-2.5 rounded-lg border transition-all ${
                      sel ? "bg-teal-500/10 border-teal-500 text-teal-400" : "border-white/8 hover:border-white/20"
                    }`}>
                    <div className="text-sm font-semibold">{s.label}</div>
                    <div className={`text-[11px] mt-0.5 ${sel ? "text-teal-400/70" : "text-slate-500"}`}>{s.detail}</div>
                  </button>
                );
              })}
            </div>
          ...
          <label className="flex gap-2 items-start cursor-pointer pt-2">
            <input type="checkbox" checked={profile.consent}
              onChange={e => setProfile({ ...profile, consent: e.target.checked })}
              className="mt-1 accent-teal-500" />
            <span className="text-xs text-slate-400 leading-relaxed">
              Acepto recibir el informe por email y futuras comunicaciones de Nektiu sobre IA.
              Puedo darme de baja en cualquier momento.
            </span>
          </label>
          {errors.consent && <p className="text-xs text-coral-500">{errors.consent}</p>}
...
function ResultsScreen({ scores, profile }: { scores: Scores; profile: Profile }) {
  const level = getLevel(scores.overall);
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 animate-fade-in">
      <div className="mb-8">
        ...
        <p className="text-slate-400 mt-3 flex items-center gap-2">
          ...
          Hemos enviado tu informe Word personalizado a {profile.email}
        </p>
      </div>

      <div className="grid md:grid-cols-2 gap-4 mb-8">
        <div className="card p-8">
          <div className="text-[11px] uppercase tracking-widest text-slate-400 mb-3">Madurez IA global</div>
          <div className="flex items-baseline gap-2 mb-4">
            <span className="font-serif text-7xl text-teal-500 leading-none tabular-nums">{scores.overall}</span>
            <span className="text-xl text-slate-400">/ 100</span>
          </div>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border" style={{ borderColor: `${level.color}40` }}>
            <span className="w-2 h-2 rounded-full" style={{ background: level.color }} />
        ...
        <div className="card p-4">
          ...
          <ResponsiveContainer width="100%" height={340}>
            <RadarChart data={radarData} margin={{ top: 20, right: 40, bottom: 10, left: 40 }}>
              ...
              <PolarAngleAxis dataKey="pillar" tick={{ fill: "#f5f7fa", fontSize: 12, fontWeight: 600 }} />
              <PolarRadiusAxis angle={90} domain={[0, 100]} tick={{ fill: "#64748b", fontSize: 10 }} stroke="rgba(255,255,255,0.08)" />
              <Radar name="Tu empresa" dataKey="Tu empresa" stroke="#00d9b2" fill="#00d9b2" fillOpacity={0.35} strokeWidth={2.5} />
              <Radar name="Media sector" dataKey="Media sector" stroke="#f5b700" fill="#f5b700" fillOpacity={0.15} strokeWidth={2} strokeDasharray="4 4" />
              <Legend wrapperStyle={{ fontSize: 12, paddingTop: 8 }} />
              <Tooltip contentStyle={{ background: "#141f3e", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 8, fontSize: 13 }} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="mb-10">
        <div className="text-[11px] uppercase tracking-widest text-slate-400 mb-3">Análisis por pilar</div>
          ...
          {PILLARS.map(p => {
            ...
            return (
              <div key={p.id} className="card p-4">
                ...
                <div className="font-serif text-3xl mt-1 tabular-nums leading-none">{score}</div>
                ...
                <div className="relative h-1 bg-white/5 rounded-full mt-3 overflow-hidden">
                  <div className="absolute top-[-2px] w-[1.5px] h-[8px] bg-gold-500" style={{ left: `${avg}%` }} />
                  <div className="h-full bg-teal-500 rounded-full" style={{ width: `${score}%` }} />
                </div>
                <div className="mt-3 inline-flex items-center gap-1 text-[11px] font-semibold" style={{ color: statusData.c }}>
                  <StatusIcon size={11} />
      ...
      <div className="mb-10">
        ...
        <p className="text-sm text-slate-400 mb-6">
          Cada acción está enfocada al pilar donde más necesitas moverte. El informe que tienes en tu email
          lo desarrolla con mucho más detalle (presupuestos, KPIs, roles).
        </p>

        <div className="grid md:grid-cols-3 gap-4">
          <ActionColumn icon={Clock} color="#ff6b6b" horizon="CORTO PLAZO" subtitle="30 días · quick wins" actions={plan.short} />
          <ActionColumn icon={Target} color="#f5b700" horizon="MEDIO PLAZO" subtitle="3 – 6 meses · estructura" actions={plan.medium} />
          <ActionColumn icon={Rocket} color="#00d9b2" horizon="LARGO PLAZO" subtitle="6 – 18 meses · transformación" actions={plan.long} />
        </div>
      </div>
      ...
      <div className="card p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-gradient-to-br from-teal-500/10 to-gold-500/5">
        ...
        <a href={`mailto:alberto@nektiu.com?subject=Sesión estratégica AI4Value - ${encodeURIComponent(profile.company)}`}
...
function ActionColumn({ icon: Icon, color, horizon, subtitle, actions }: {
  icon: any; color: string; horizon: string; subtitle: string;
  actions: Array<{ pillar: string; text: string }>;
}) {
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2.5 mb-1">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${color}26` }}>
          <Icon size={15} style={{ color }} />
        </div>
        <div>
          <div className="text-[10px] font-bold tracking-widest" style={{ color }}>{horizon}</div>
        ...
        {actions.map((a, i) => (
          <div key={i} className="flex gap-2 items-start">
            ...
            <div className="text-[13px] leading-relaxed">
              <span className="text-[9px] font-bold tracking-wider uppercase mr-1.5" style={{ color }}>
                {a.pillar}
              </span>
